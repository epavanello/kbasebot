...
import { createEdgeRouter, RequestHandler, NextHandler } from "next-connect";
...
export const respondToUrlVerification: RequestHandler<NextApiRequest, NextApiResponse> = (req, res, next) => {
    if (req.body && req.body.type && req.body.type === "url_verification") {
        NextResponse.json({challenge: req.body.challenge});
        return;
    }
    next();
};
...
export default class NextConnectReceiver implements Receiver {
    ...
    public constructor({
                           ...
                           endpoints = { events: "/api/integrations/slack/events", commands: "/api/integrations/slack/commands", actions: "/api/integrations/slack/actions" },
            ...
            this.router.get(installPath, async (_req: NextApiRequest, res: NextApiResponse, next: NextHandler) => {
                try {
                    ...
                    if (installerOptions.directInstall) {
                        ...
                        NextResponse.send(renderHtml(url));
                    }
                } catch (error) {
                    next(error);
                }
        ...
        try {
            ...
            if (storedResponse !== undefined) {
                if (typeof storedResponse === "string") {
                    NextResponse.send(storedResponse);
                } else {
                    NextResponse.json(storedResponse);
                }
            ...
            new Response(getDevErrorMessage(e, e?.message || "slack integration error"), {
                status: 500,
            });
            throw err;
        }
        ...
        try {
            ...
            if (error) {
                if (error instanceof ReceiverAuthenticityError) {
                    ...
                    return new Response(getDevErrorMessage(error, "slack integration error"), {
                        status: 401,
                    });
                }
